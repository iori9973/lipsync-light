...
public class PreserveOriginalForBlackTests
{
    private static (GameObject avatarRoot, MeshRenderer renderer, Material mat) BuildHierarchyWithMaterial(
        string rootName, string childName)
    {
        var avatarRoot     = new GameObject(rootName);
        var childTransform = new Transform { name = childName, parent = avatarRoot.transform };
        ...
        var mat = new Material();
        renderer.sharedMaterials = new[] { mat };
        return (avatarRoot, renderer, mat);
    }

    [Fact]
    public void True_WithNonBlackMaterialColor_AddsUseEmission()
    {
        // 元マテリアルが非黒の _EmissionColor を持つとき、Off クリップが元色にフォールバックし
        // _UseEmission が有効化されることを確認する
        var (avatarRoot, renderer, mat) = BuildHierarchyWithMaterial("Root", "Body");
        mat.SetColor("_EmissionColor", new Color(1, 0, 0, 1)); // red
        mat.AddProperty("_UseEmission");
        ...
        var clip  = LipsyncLightBuilder.CreateEmissionClip(
            avatarRoot, targets, _ => Color.black, "OffClip",
            preserveOriginalForBlack: true);
        var props = AnimationUtility.GetCurveBindings(clip).Select(b => b.propertyName).ToList();

        // 元色（red）にフォールバックするため _UseEmission が追加される
        Assert.Contains("material._UseEmission", props);
    }

    [Fact]
    public void False_DoesNotFallback()
    {
        // preserveOriginalForBlack = false（デフォルト）のとき、元マテリアルに非黒色があっても
        // フォールバックせず色は黒のまま → _UseEmission は追加されない
        var (avatarRoot, renderer, mat) = BuildHierarchyWithMaterial("Root", "Body");
        mat.SetColor("_EmissionColor", new Color(1, 0, 0, 1)); // red
        mat.AddProperty("_UseEmission");
        ...
        var clip  = LipsyncLightBuilder.CreateEmissionClip(
            avatarRoot, targets, _ => Color.black, "OffClip"); // default = false
        var props = AnimationUtility.GetCurveBindings(clip).Select(b => b.propertyName).ToList();
    ...
    [Fact]
    public void True_WithBlackMaterialColor_NoFallback()
    {
        // 元マテリアルも黒の場合はフォールバックしても色が変わらず、_UseEmission は追加されない
        var (avatarRoot, renderer, mat) = BuildHierarchyWithMaterial("Root", "Body");
        mat.SetColor("_EmissionColor", Color.black); // 元マテリアルも黒
        mat.AddProperty("_UseEmission");
    ...
    [Fact]
    public void True_PerProperty_EachPropertyFallsBackIndependently()
    {
        // 複数プロパティがある場合、非黒のプロパティのみ有効化フラグが追加される
        var (avatarRoot, renderer, mat) = BuildHierarchyWithMaterial("Root", "Body");
        mat.SetColor("_EmissionColor",    new Color(1, 0, 0, 1)); // 非黒
        mat.SetColor("_Emission2ndColor", Color.black);            // 黒のまま
        mat.AddProperty("_UseEmission");
        ...
        var targets = new List<EmissionTarget>
        {
            new EmissionTarget
            {
                Renderer      = renderer,
                MaterialIndex = 0,
                PropertyNames = new List<string> { "_EmissionColor", "_Emission2ndColor" },
            }
        };
        ...
        Assert.Contains("material._UseEmission", props);          // 非黒 → 追加
        Assert.DoesNotContain("material._UseEmission2nd", props); // 黒 → 追加されない
    }
}
